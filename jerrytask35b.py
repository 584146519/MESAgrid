...
def makemesadir(a,content='grid',start=0,step=1):#创建一维网格文件夹
    '传入网格数量,网格的名字'
    a=int(a)
    ...
    grids=[]
    for i in range(a):
        os.system('cp -r ./mesabasic ./mesagrid/'+f"{gridname}_{start+step*i:.{decimal}f}")
        grids+=['./mesagrid/'+f"{gridname}_{start+step*i:.{decimal}f}"]
...
def mkmesadir2D(a, b, name='grid', start1=0, step1=1, start2=0, step2=1):#创建二维网格文件夹
    ...
    for i in range(a):
        for j in range(b):
            dirname = f'{gridname}_{start1+step1*i:.{decimal1}f}_{start2+step2*i:.{decimal2}f}' 
...
def startall(setcore=1,Executable_file='rn'):#开始的网格数量
    setcore=int(setcore)
    # 获取CPU的核心数量
    cores = multiprocessing.cpu_count()
    print("Number of CPU cores:", cores)
...
def restartall(cpu_percent=50,setcore=0):#网格数，re 的步数位置
...
def nethr(delangle=80):
    ...
    for i in paths:
        ...
        try:
            a,b,ts=timescaleo(order,i,4.2,4.8)
        except TypeError:
            pass
        teff,Lum=h['log_Teff'],h['log_L']
        ...
        try:
            bas,ket,time_scale=timescaleo(order,i)
            u,p,o=timescale(i)
        ...
        k=str(i).split("_")[-1]
        print(k,i)
        # plt.savefig(f'./mesagrid/HR/He={He:.3f}_'+str(str(k)[:-5])+'.png')
        plt.savefig(f'./mesagrid/HR/{float(str(k)[:-5]):.6f}_He={He:.3f}.png')
...
def graphtso(delangle=80):
    ...
    for i in paths:
        ...
        x.append(float(parts[0].rsplit('_', 1)[-1]))
        y.append(float(parts[1].rsplit('.', 1)[0]))
        ts.append(timescaleo(order,i)[2])
    ...
    colorbar = plt.colorbar(scatter)
    colorbar.set_label('time scale')
    plt.savefig(os.path.join(path, 'timescaleo.png'), dpi=300)
    plt.show()

def create_gui_for_function(frame, func, column):
    subframe = tk.Frame(frame)
    subframe.grid(row=column // 4, column=column % 4, padx=10, pady=10, sticky='nw')
    def invoke():
        args = {name: entry.get() for name, entry in entries.items()}


    tk.Label(subframe, text=f"Function: {func.__name__}").pack()
...
if __name__ == "__main__":
    root = tk.Tk()
    root.title("Function GUI Generator")
    root.geometry("1050x800")  # 设置初始大小
    ...
    # 排除列表
    exclude_functions = ['create_gui_for_function','timescaleo','timescale','replaceline2','getvalue','glob','update_value_in_file','count_decimal_places','版本变化']
