...
for dir in dir_list:
    ...
    with open(nohup_out, "a") as outfile:
        ...
        # 启动进程
        process = subprocess.Popen(exec_cmd, stdout=outfile, stderr=subprocess.STDOUT, cwd=cwd)
    ...
    with open("nohup.out", "a") as outfile:
        ...
        process = subprocess.Popen(exec_cmd, stdout=outfile, stderr=subprocess.STDOUT)

def resumeall(setcore=1, Executable_file='re'):
    setcore = int(setcore)
    if setcore != 0:
        os.environ['OMP_NUM_THREADS'] = str(setcore)
...
def gethistorydata(history_dirname = 'LOGS'):
    ...
    for i in glob('mesagrid/*/'):
        ...
        # 构造原始和目标文件的完整路径
        original_file = os.path.join(i, history_dirname, 'history.data')
