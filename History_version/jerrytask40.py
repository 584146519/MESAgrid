...
exclude_functions = ['create_gui_for_function','find_value_line','timescale','replaceline2','getvalue','glob','update_value_in_file','count_decimal_places','版本变化','process_HR','on_mouse_wheel','main','call_function_by_number','get_functions','display_functions','change_log','timescale','timescaleo']
...
def grid1D(number_of_1Dgrid , parameter_name ='grid', start=0, step=1):
    ...
    for i in range(a):
        ...
        grids.append(dst)
    replaceline(parameter_name ,start,step)
...
def grid2D(number_of_2Dgrid_rows, number_of_2Dgrid_columns, parameter_name_1='grid1', parameter_name_2='grid2', start1=0, step1=1, start2=0, step2=1):
    """创建二维网格文件夹"""
    global gridname 
    global grids
    a, b = int(number_of_2Dgrid_rows), int(number_of_2Dgrid_columns)
    ...
    for i in range(a):
        for j in range(b):
            dirname = f'{parameter_name_1}_{start1+step1*i:.{decimal1}f}_{parameter_name_2}_{start2+step2*j:.{decimal2}f}'
...
def replaceline2D(a, b,parameter_name_1,parameter_name_2, start1=0, step1=1, start2=0, step2=1):#对二维网格进行修改
