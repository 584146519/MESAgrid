...
exclude_functions = ['create_gui_for_function', 'find_value_line', 'timescale', 'replaceline2', 'getvalue', 'glob', 'update_value_in_file',
...
def grid2D(number_of_2Dgrid_rows, number_of_2Dgrid_columns,
    ...
    若 start 参数中包含 "d"，则认为采用科学计数法。
    对于每个方向：
      - 计算的新数值使用 get_value(start, step, index) 得到 float 值，
      - 若 sci_format 为 True，则调用 simple_fortran_format 生成简化表示（如 "1d-2"），
        否则用 f 格式保留指定小数位数。
    """
    ...
    for i in range(a):
        for j in range(b):
            # 计算数值
            value1 = get_value(start1, step1, i)
            value2 = get_value(start2, step2, j)
            # 根据 sci_format 决定文件夹名称字符串
            if sci_format1:
                value1_str = simple_fortran_format(value1)
            else:
                value1_str = f"{value1:.{decimal1}f}"
            if sci_format2:
                value2_str = simple_fortran_format(value2)
            else:
                value2_str = f"{value2:.{decimal2}f}"
            dirname = f'{parameter_name_1}_{value1_str}_{parameter_name_2}_{value2_str}'
            dir_path = os.path.join(base_dir, dirname)
            os.makedirs(dir_path, exist_ok=True)
            shutil.copytree('mesabasic', dir_path, dirs_exist_ok=True)
            grids.append(dir_path)
    
    # 更新每个网格中的 inlist 文件中的参数值
    for i in range(a):
        for j in range(b):
            filename = os.path.join(grids[i * b + j], inlist_name)
            value1 = get_value(start1, step1, i)
            value2 = get_value(start2, step2, j)
            update_value_in_file(filename, parameter_name_1, value1, decimal1, sci_format=sci_format1)
            update_value_in_file(filename, parameter_name_2, value2, decimal2, sci_format=sci_format2)
...
def changevalue2D(a, b,parameter_name_1,parameter_name_2, start1=0, step1=1, start2=0, step2=1):#对二维网格进行修改
    ...
    for i in range(a):
        for j in range(b):
            ...
            b1 = start1 + step1 * i
            b2 = start2 + step2 * j
            update_value_in_file(filename, parameter_name_1, b1,decimal1)
            update_value_in_file(filename, parameter_name_2, b2,decimal2)
    ...
    for i in glob('./mesagrid/*/'):
        try:
            ...
            with open(os.path.join(i, "nohup.out"), "a") as outfile:
                subprocess.Popen(exec_cmd, cwd=i, stdout=outfile, stderr=subprocess.STDOUT,    start_new_session=True, close_fds=True )
...
def resume_mesaall(setcore=1, Executable_file='re'):
    setcore = int(setcore)
    if setcore != 0:
        os.environ['OMP_NUM_THREADS'] = str(setcore)
    ...
    for i in glob('./mesagrid/*/'):
        try:
            # 给予文件执行权限
            subprocess.run(['chmod', '700', 'mk', 're', 'rn', 'star'], cwd=i)

            # 构造执行 mk 的命令
            subprocess.run(['./mk'], cwd=i)

            # 检查是否存在photos文件夹
            photos_exists = os.path.exists(os.path.join(i, 'photos'))
            
            # 决定运行哪个可执行文件
            exec_file = Executable_file if photos_exists else 'rn'
            
            # 构造并执行主程序的命令
            exec_cmd = ['nohup', 'python3', exec_file] if exec_file.endswith(".py") else ['nohup', f'./{exec_file}']
            with open(os.path.join(i, "nohup.out"), "a") as outfile:
                subprocess.Popen(exec_cmd, cwd=i, stdout=outfile,stderr=subprocess.STDOUT ,    start_new_session=True,  close_fds=True)
